...
import Analysis.ClusterUtils;
import Classification.ThermalZones;
import Output.ClusterSummary;
import Output.ComfortIndices;
import Performance.ClusteringPerformance;
import ReadFilter.Record;
import ReadFilter.YearComparator;
...
public class ClusteringOutput {
	...
	public ClusteringOutput(JavaRDD<Record> records,String output,SparkSession spark,List<ClusteringPerformance> performance,ThermalZones thermalzones,int numClusters,Vector[] clusterCenters,String name ) {
    	...
    	//drop all in one file
    		records.map(f->f.toJSONString()).saveAsTextFile(output+"/stats/all/");
